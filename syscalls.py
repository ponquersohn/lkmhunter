...
class Syscall:
    ...
    def __str__(self):
        offset = 0
        return f"number: {self.number}, name: {self.name}, address: {self.address:x}, entry_point: {self.entry_point - offset}"
...
class SyscallTable:
    ...
    def get_syscall(self, syscall_number, relocate=True):
        ...
        syscall_address = self.address + 4 * syscall_number
        self.image.seek(syscall_address)
        syscall = Syscall(
            syscall_number,
            address=syscall_address,
            entry_point=self.image.read(4),
            offset=self.offset,
        )
        return syscall
