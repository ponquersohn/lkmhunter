...
syscall_enum = {
    ...
    10: ("mprotect", "sys_mprotect", "mm/mprotect.cs"),
...
class Syscall:
    @property
    def relocated_entry_point(self):
        return self.entry_point - self.offset

    def __init__(self, number, address, entry_point, offset=0):
        self.address = address
        self.number = number
        self.offset = offset
        self.entry_point = entry_point
        if len(syscall_enum[number]) == 3:
            self.name, self.entry_point_name, self.implementation = syscall_enum[number]
        else:
            self.name, self.implementation = syscall_enum[number]
            self.entry_point_name = self.name

    def __str__(self):
        offset = 0
        return f"number: {self.number}, name: {self.name}, address: {self.address:x}, entry_point: {self.entry_point}, offsetted_entry_point:{self.relocated_entry_point}"


class SyscallTable:
    def __init__(self, image: BufferedReader, address, length=None, offset=0):
        self.offset = offset
        self.address = address
        self.image: BufferedReader = image
        if not length:
            length = len(syscall_enum)
        self.length = length

    def iterate(self, relocate=True):
        for x in range(0, self.length):
            yield self.get_syscall(x)

    def print_table(self, relocate=True):
        syscall: Syscall = None
        for syscall in self.iterate(relocate):
            print(syscall)

    def get_syscall(self, syscall_number, relocate=True):
        if syscall_number > self.length:
            raise Exception("syscall out of range for this table.")
        syscall_address = self.address.physical_address + 8 * syscall_number
        self.image.seek(syscall_address)
        syscall = Syscall(
            syscall_number,
            address=syscall_address,
            entry_point=int.from_bytes(self.image.read(4), "little"),
            offset=self.offset,
        )
        return syscall
