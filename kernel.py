...
class KernelImage:
    ...
    def load_sys_call_table(self, length=314):
        ...
        sys_call_table = SyscallTable(
            self.image, syscall_table_address.address, offset=self.offset, length=length
        )
    ...
    def calculate_offset(self, kernel_image: "KernelImage"):
        ...
        this_address = self.get_symbol_address(symbol_to_compare)
        other_address = kernel_image.get_symbol_address(symbol_to_compare)
        self.offset = this_address - other_address
    ...
    def compare_syscall_table(self, other):
        return
